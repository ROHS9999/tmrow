 ...
 * (http://www.csecho.ca/wp-content/themes/twentyeleven-csecho/cardiomath/?eqnHD=stress&eqnDisp=mvo2tm).
 * It allows you to estimate the METs and VO2 given the treadmill speed
 * and the grade. In this case allows to estimate the METs given the
 * walking speed and the slope of the streets.
 *
 * Speed = WalkingSpeed * 26.8 Horizontal Component (HC) = Speed * 0.1
 *
 * The slope is equivalent to the treadmill grade, which is basically a
 * measure of the height distance for every 100 horizontal distance.
 * e.g. A one in 100 gradient = 1%, and a rise of 15 meters for every
 * 100 meters is a 15 % grade. To calculate the percent grade, simply
 * divide the rise by the run and multiply by 100. For example, if you
 * raise the belt so that the front is 0.5 feet higher than the back,
 * and the front and the back are separated by 8 feet along the floor,
 * then the percent grade is (0.5/8) * 100 = 6.25 percent.
 *
 *  Slope = raise/distance*100
 *
 *  Vertical Component (VC) = Speed * 1.8 * Slope VO2 = HC + VC + Rest =
 *  HC + VC + 3.5 mL/kg/min Mets = (VO2/3.5)
 *
 *   Speed [mph] Slope [%]
 */
...
public class Effort {
     ...
     * 
     * @param distance A double representing the distance of travelling from place0
     *                 to place1
     * @param time     A double representing the time of travelling from place0 to
     *                 place1
...
//		}
    ...
    /**
     * Calories estimation of travelling from place0 to place1 EE =
     * (0.1*S+1.8*S*G+3.5)*BM*t*0.005 where EE is walking energy expenditure
     * (kilocalories) S is walking speed (meters/minute) G is grade (slope) in
     * decimal form (e.g., 0.02 for 2% grade) BM is traveller�s body weight
     * (kilograms) t is walking time (minutes)
     * 
     * @return EE
     */
