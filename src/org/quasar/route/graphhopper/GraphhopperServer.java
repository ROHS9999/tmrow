...
public class GraphhopperServer {
    ...
    private GraphHopper hopper = initializeHopper();
    ...
    public GraphhopperServer(RouteRequest request) {
        ...
        //hopper.setGraphHopperStorage(new GraphHopperStorage(new RAMDirectory(), EncodingManager.create(VEHICLE), true, true, 1));
    ...
    public GraphHopper initializeHopper(){
        org.apache.log4j.BasicConfigurator.configure();
        ...
        // see docs/core/profiles.md to learn more about profiles
        hopper.setProfiles(new Profile(VEHICLE).setVehicle(VEHICLE).setWeighting(WEIGHTING));
        ...
        //hopper.setGraphHopperStorage(new GraphHopperStorage(new RAMDirectory(), EncodingManager.create(VEHICLE), true, false, 1));
     ...
     *
     * @param selectedPoints by using the list of POIs IDs
     * @return
     */
    public LinkedList<PointOfInterest> getSelectedPOIs(LinkedList<Integer> selectedPoints) {
        ...
        // NOTA!! SE O NUMERO DE POIS FOR IGUAL AO MAX_POIS_TO_VISIT ENT�O NEM � PRECISO
        // FAZER AS VERIFICA��ES ABAIXO, � S� RETORNAR A LISTA DE PONTOS DE INTERESSE
        ...
        if (selectedPOIs.size() < MAX_POIS_TO_VISIT) {
            ...
            if (timeVisitEstimation(selectedPOIs) < request.getVisitationTime()) {
                ...
                // NOTA!! SE AS CATEGORIAS VIEREM VAZIAS E N�O TIVER CHOVIDO, N�O VALE A PENA IR
                // PARA A RECOMENDA��O PORQUE S�
                // DEVEM SER INCLU�DOS OS POIS ESCOLHIDOS PELO USER
...
//https://discuss.graphhopper.com/t/how-to-use-graphhopper-routing-requests-with-profile-in-gh-1-0/5565
            ...
            while (j < poiList.size()) {
                ...
                if (rsp.hasErrors()) {
                    ...
                    for (Throwable tw : rsp.getErrors())
                        System.out.println("Rsp error:" + tw.toString());
                    return null;
                }
                System.out.println("A hora de abertura � " + tempOpenHours.get(i) + " e a hora de fecho � "
                        + tempCloseHours.get(i));
                System.out.println("S�o " + calendar2.get(Calendar.HOUR_OF_DAY) + " horas");
     ...
     * it
     *
     * @param poiScenariosList A list of lists representing all the different
     *                         possibilities of traveling from the origin to the
     *                         destination and passing through all the
     *                         chosen/suggested POIs
    ...
    public LinkedList<LinkedList<ResponsePath>> dividedRequest(LinkedList<LinkedList<GHPoint>> listOfScenarios) {
        ...
        for (LinkedList<GHPoint> ghPointList : listOfScenarios) {
            ...
            while (j < ghPointList.size()) {
                ...
                GHRequest req = new GHRequest(begin, end).
                        setProfile(VEHICLE).
                        setAlgorithm(ALGORITHM).
                        setLocale(Locale.US);
        ...
        System.out.println("N�mero de paths: " + path.size());
        return path;
    }
    ...
    public List<ResponsePath> compareAlternativeRoutes(LinkedList<LinkedList<ResponsePath>> pathAlternatives) {
        ...
        for (LinkedList<ResponsePath> diffPaths : pathAlternatives) {
            ...
            for (ResponsePath path : diffPaths) {
                ...
                // CRIAR C�PIA DO ResponsePath
                // CRIAR ESTRUTURA DOS EDGES DO ResponsePath
                // CICLO DE ITERA��O SOBRE O ResponsePath
        ...
        System.out.println("SIZE OF ResponsePath IS: " + sortedPaths.size());
        return sortedPaths;
    }
    ...
    private LinkedList<PointOfInterest> getOrderOfPOIs() {
        ...
        System.out.println("O tamanho do TEMP � " + temp.size());
    ...
    public double evaluateBudget(List<PointOfInterest> selectedPoints) {
        ...
        if (selectedPoints.size() != 0) {
            ...
            if (price >= request.getBudget()) {
                System.out.println("Percurso imposs�vel de ser efetuado");
            }
        }
    ...
    public Route createRouteForResponse() {
        ...
        System.out.println("O tamanho de timedPOIs �: " + timedPOIs.size());
